...
int CNetObjHandler::ValidateObj(int Type, void *pData, int Size)
{
	...
	case NETOBJTYPE_PROJECTILE:
	{
		...
		if((int)sizeof(*pObj) > Size) return -1;
		pObj->m_Type = ClampInt("m_Type", pObj->m_Type, 0, NUM_WEAPONS-1);
		return 0;
	}
	...
	case NETOBJTYPE_PICKUP:
	{
		...
		if((int)sizeof(*pObj) > Size) return -1;
		pObj->m_Type = ClampInt("m_Type", pObj->m_Type, 0, max_int);
	...
	case NETOBJTYPE_DDNETCHARACTER:
	{
		...
		if((int)sizeof(*pObj) > Size) return -1;
		pObj->m_Jumps = ClampInt("m_Jumps", pObj->m_Jumps, 0, 255);
		pObj->m_StrongWeakID = ClampInt("m_StrongWeakID", pObj->m_StrongWeakID, 0, MAX_CLIENTS-1);
		return 0;
	}
	...
	case NETOBJTYPE_DDNETPLAYER:
	{
		...
		if((int)sizeof(*pObj) > Size) return -1;
		pObj->m_AuthLevel = ClampInt("m_AuthLevel", pObj->m_AuthLevel, AUTHED_NO, AUTHED_ADMIN);
		return 0;
	}
	...
	case NETOBJTYPE_SWITCHSTATE:
	{
		...
		if((int)sizeof(*pObj) > Size) return -1;
		pObj->m_NumSwitchers = ClampInt("m_NumSwitchers", pObj->m_NumSwitchers, 0, 256);
		return 0;
	}
...
void *CNetObjHandler::SecureUnpackMsg(int Type, CUnpacker *pUnpacker)
{
	...
	{
		CNetMsg_Sv_DDRaceTimeLegacy *pMsg = (CNetMsg_Sv_DDRaceTimeLegacy *)m_aMsgData;
		(void)pMsg;
	...
	{
		CNetMsg_Sv_RecordLegacy *pMsg = (CNetMsg_Sv_RecordLegacy *)m_aMsgData;
		(void)pMsg;
	...
	{
		CNetMsg_Sv_TeamsStateLegacy *pMsg = (CNetMsg_Sv_TeamsStateLegacy *)m_aMsgData;
		(void)pMsg;
	} break;
	
	case NETMSGTYPE_CL_SHOWOTHERSLEGACY:
	{
		CNetMsg_Cl_ShowOthersLegacy *pMsg = (CNetMsg_Cl_ShowOthersLegacy *)m_aMsgData;
		(void)pMsg;
	...
	{
		CNetMsg_Cl_ShowDistance *pMsg = (CNetMsg_Cl_ShowDistance *)m_aMsgData;
		(void)pMsg;
	...
	case NETMSGTYPE_CL_SHOWOTHERS:
	{
		...
		(void)pMsg;
		pMsg->m_Show = pUnpacker->GetInt();
		if(pMsg->m_Show < 0 || pMsg->m_Show > 2) { m_pMsgFailedOn = "m_Show"; break; }
	} break;
...
void RegisterGameUuids(CUuidManager *pManager)
{
	...
	pManager->RegisterName(NETOBJTYPE_DDNETPROJECTILE, "projectile@netobj.ddnet.tw");
